...
plot_sankey <- function(lrobj_tbl,
                        ...
                        tfflag = TRUE,
                        score_col = "LRScore") {
  lrobj_tbl <- lrobj_tbl %>% filter(type_gene_A == "Ligand" &
  ...
  if (!is.null(target)) {
    if (length(stringr::str_split(target, "\\|")[[1]]) >
      ...
      if (target_type == "R") {
        if (length(which(grepl("\\|", lrobj_tbl$gene_B))) >
          0) {
          data <- lrobj_tbl %>% filter(gene_B == !!target)
        } else {
          target <- stringr::str_split(target, "\\|")[[1]][[1]]
          data <- lrobj_tbl %>% filter(gene_B == !!target)
        }
      } else if (target_type == "L") {
        if (length(which(grepl("\\|", lrobj_tbl$gene_A))) >
          0) {
          data <- lrobj_tbl %>% filter(gene_A == !!target)
        } else {
          target <- stringr::str_split(target, "\\|")[[1]][[1]]
          data <- lrobj_tbl %>% filter(gene_A == !!target)
        }
  ...
  if (!is.null(ligand_cluster)) {
    if (!is.null(receptor_cluster)) {
      data <- data[(data$source %in% ligand_cluster) &
        (data$target %in% receptor_cluster), ]
    } else {
      data <- data[(data$source %in% ligand_cluster), ]
    }
  }
  ...
  if (dim(data)[1] >= 1) {
    ...
    print(ggplot2::ggplot(tmp, aes(
      ...
      ggalluvial::geom_alluvium(aes(
        fill = .data[[score_col]],
        color = "b"
      ), width = 1 / 12, discern = FALSE) +
      ...
      ggplot2::scale_fill_gradient2(
        ...
        high = colorBlindness::Blue2DarkOrange18Steps[14],
        midpoint = 0
      ) +
