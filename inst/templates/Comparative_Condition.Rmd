...
## Differential Gene_Cell Gene_Cell Interaction Plot (GGI): *Highest degree from Largest Component*
...
## Differential Gene_Cell Gene_Cell Interaction Plot (GGI): *Articulation point from Largest Component*
...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE)){
    ...
    sel_graph <- induced.subgraph(curr_net, V(curr_net)[comp$membership==sel])
    plot_articulation(sel_graph,color = all_data@colors)
  }
}
```
