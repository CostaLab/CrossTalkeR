...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE)){
    ...
    plot_cci(curr_net,
               ...
               paste0(curr,' CCI'),all_data@coords[V(curr_net)$name,],leg=TRUE,low=low,high = high,emax = tmp_max,log = T)
  }
}
```
...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE)){
    ...
    p1 <- ggplot2::ggplot(in_all,ggplot2::aes(x=Freq,y=reorder(Var1,abs(Freq)),fill=Expression))+
      ...
      ggplot2::scale_fill_manual(values=c('navy','red'))+
...
## Differential Gene_Cell Gene_Cell Interaction Plot (GGI): *Highest degree connector candidate from Largest Component*

```{r fig.align='center', fig.height=10, fig.width=16,message=FALSE, warning=FALSE,echo=F}
for(i in 1:length(names(all_data@tables))){
  curr <- names(all_data@tables)[i]
  if(str_detect(curr, '_x_', negate = FALSE)){
...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE)){
    ...
    in_up <- tibble::tibble(genes = names(in_deg_up), values=as.array(in_deg_up))
    in_down <- tibble::tibble(genes = names(in_deg_down), values=as.array(in_deg_down))
    in_deg_data_up <-dplyr::top_n(in_up, 15, values)
    in_deg_data_down <- dplyr::top_n(in_down, 15, values)
    ...
    out_up <- tibble::tibble(genes = names(out_deg_up), values=as.array(out_deg_up))
    out_down <- tibble::tibble(genes = names(out_deg_down), values=as.array(out_deg_down))
    out_deg_data_up <-dplyr::top_n(out_up, 15, values)
    out_deg_data_down <- dplyr::top_n(out_down, 15, values)
