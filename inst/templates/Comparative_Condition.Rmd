...
```{r fig.align='center', fig.height=7, fig.width=12,message=FALSE, warning=FALSE,echo=F}
...
## Differential Gene_Cell Gene_Cell Interaction Plot (CGI): *Highest degree connector candidate from the Largest Component*
...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE)){
    ...
    in_deg_data_up <-dplyr::top_n(in_up, 5, values)
    in_deg_data_down <- dplyr::top_n(in_down, 5, values)
    ...
    out_deg_data_up <-dplyr::top_n(out_up, 5, values)
    out_deg_data_down <- dplyr::top_n(out_down, 5, values)
