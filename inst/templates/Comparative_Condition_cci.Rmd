...
for(i in 1:length(names(all_data@tables))){
  ...
  if(str_detect(curr, '_x_', negate = FALSE) & !str_detect(curr, 'ggi', negate = FALSE)){
    ...
    curr_net <- subgraph.edges(all_data@graphs[[curr]],E(all_data@graphs[[curr]])[match(all_data@stats[[curr]]$columns_name[all_data@stats[[curr]]$p<=0.05]
,paste(h,f,sep = '_'),nomatch = F))])
      low <- thr
