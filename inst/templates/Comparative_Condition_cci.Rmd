...
for(i in names(all_data@stats)){
     print(ggplot(all_data@stats[[i]],aes(x=lodds,y=reorder(columns_name,lodds),fill=ifelse(lodds>0,T,F)))+
        geom_bar(stat='identity')+
        geom_text(aes(label=p.signif), vjust=-0.3, size=3.5,hjust=0.3)+
        scale_fill_manual(values = c(Blue2DarkOrange18Steps[4],Blue2DarkOrange18Steps[14]))+    
        xlab("Log Odds Ratio")+
