...
```{r annotdiff, fig.align='center', fig.height=7, fig.width=10,warning=FALSE,echo=False}
all_data <- readRDS(path)
for(i in names(all_data@annot)){
  if( grepl('_x_',i) & grepl('ggi',i) ){
    ...
    mat[is.na(mat)]=1
    p1<-Heatmap(-log10(mat), col =c('#FFFFFF',Blue2DarkOrange18Steps[11:18]), column_title =i,name='enrich signficance')
    draw(p1)
   }
   else{
...
```{r fig.align='center', fig.height=5, fig.width=10 ,message=FALSE, warning=FALSE, echo=False}
all_data <- readRDS(path)
