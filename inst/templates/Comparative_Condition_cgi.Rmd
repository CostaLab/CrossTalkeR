...
genes_table = names(all_data@tables)[which(grepl("_x_", names(all_data@tables)))]

if (("Transcription Factor" %in% all_data@tables[[genes_table]]$type_gene_A) | ("Transcription Factor" %in% all_data@tables[[genes_table]]$type_gene_A)) {
  PCA_TF_flag <- TRUE
}else {
  PCA_TF_flag <- FALSE
}
...
maxdim <- function(tbls,all_data){
     return(colnames(all_data@pca[[tbls]]$x) >= 4)
}
if (sum(unlist(lapply(pca_table1,maxdim,all_data))) == length(names(pca_table1))) {
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = PCA_TF_flag)
  plot_pca_LR_comparative(all_data, dims = c(3, 4), ret = T, ggi = TRUE, include_tf = PCA_TF_flag)
}else{
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = PCA_TF_flag)
}
...
if (sum(unlist(lapply(pca_table1,maxdim,all_data))) >= length(pca_table1)) {
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = FALSE, gene_types = "LR")
  plot_pca_LR_comparative(all_data, dims = c(3, 4), ret = T, ggi = TRUE, include_tf = FALSE, gene_types = "LR")
}else{
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = FALSE, gene_types = "LR")
}
...
pca_table = names(all_data@pca)[which(
  grepl("_x_", names(all_data@pca))
    & grepl("_ggi", names(all_data@pca)))]
...
if (sum(unlist(lapply(pca_table,maxdim,all_data)))>= length(pca_table)) {
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = PCA_TF_flag, gene_types = "TF")
  plot_pca_LR_comparative(all_data, dims = c(3, 4), ret = T, ggi = TRUE, include_tf = PCA_TF_flag, gene_types = "TF")
}else{
  plot_pca_LR_comparative(all_data, dims = c(1, 2), ret = T, ggi = TRUE, include_tf = PCA_TF_flag, gene_types = "TF")
}
