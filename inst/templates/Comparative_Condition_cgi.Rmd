...
for (i in 1:length(names(all_data@pca))) {
  ...
  if (str_detect(curr, '_x_', negate = FALSE) & str_detect(curr, '_ggi', negate = FALSE)) {
    ...
    for (i in 2:dim(all_data@pca[[curr]]$x)[2]) {
      x <- max(abs(all_data@pca[[curr]]$x[, i - 1]))
      y <- max(abs(all_data@pca[[curr]]$x[, i]))
      z_x <- all_data@pca[[curr]]$x[, i - 1]
      z_y <- all_data@pca[[curr]]$x[, i]
      ver_zx <- ifelse(abs(z_x) >= (2 * all_data@pca[[curr]]$sdev[i - 1]), 1, 0)
      ver_zy <- ifelse(abs(z_y) >= (2 * all_data@pca[[curr]]$sdev[i]), 1, 0)
...
```
#### PCA GCI Ligand and Receptor Genes
...
#### PCA GCI Transcription Factor Genes
...
```{r annotdiff, fig.align='center', fig.height=7, fig.width=10,warning=FALSE,echo=F,eval=FALSE}
all_data <- readRDS(path)
...
## Transcription Factor Sankey Plot: Transcription Factor-based intra cellular interactions

```{r fig.align='center', fig.height=5, fig.width=10 ,message=FALSE, warning=FALSE, echo=F,eval=TRUE}
all_data <- readRDS(path)
#if (!is.null(params$obj3)) {
