...
```{r annotsingle, fig.align='center', fig.height=7,fig.width=10,warning=FALSE,echo=F}
path <-params$obj1
out <- c()
out1 <- list()
...
for(i in 1:length(names(all_data@tables))){
     ...
     if(!str_detect(curr, '_x_', negate = FALSE)){
         ...
         knit_expanded <- paste0("```{r results='",rmd_title,"', echo=FALSE,warning=F}
                                 \n\n                          datatable(all_data@tables[['",curr,"']][,c('Ligand.Cluster','Receptor.Cluster','Ligand','Receptor' ,'MeanLR')],caption='",curr,"', extensions = 'Buttons', options = list(
        dom = 'Bfrtip',
